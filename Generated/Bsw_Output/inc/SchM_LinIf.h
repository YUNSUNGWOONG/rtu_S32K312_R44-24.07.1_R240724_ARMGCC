...
#define SchM_Enter_LinIf_CHANNEL_STATUS_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_LinIf_CHANNEL_STATUS_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_LinIf_WAKEUP_CONFCOUNT_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_LinIf_WAKEUP_CONFCOUNT_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_LinIf_SLEEP_REQUEST_STATUS_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_LinIf_SLEEP_REQUEST_STATUS_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_LinIf_SPORADIC_STATUS_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_LinIf_SPORADIC_STATUS_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_LinIf_STATUS_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_LinIf_STATUS_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
