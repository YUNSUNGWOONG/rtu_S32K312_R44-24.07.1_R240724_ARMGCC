...
extern FUNC(EcuM_StateType, RTE_CODE) SchM_Mode_WdgM_modeNotificationPort_EcuMode(P2VAR(EcuM_StateType, AUTOMATIC, RTE_APPL_DATA) previousMode,P2VAR(EcuM_StateType, AUTOMATIC, RTE_APPL_DATA) nextMode);
/*LDRA_INSPECTED 580 S ; Source: MISRA:2012 R.1.1,R.20.4 */
/*LDRA_INSPECTED 628 S ; Source: MISRA:2012 R.2.5 */
/*LDRA_INSPECTED 338 S ; Source: MISRA:2012 R.20.1 */
...
#define SchM_Enter_WdgM_GLOBALSUPVSNSTATUS_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_WdgM_GLOBALSUPVSNSTATUS_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_WdgM_FIRSTEXPIREDSEID_MULTICORE_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_WdgM_FIRSTEXPIREDSEID_MULTICORE_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
