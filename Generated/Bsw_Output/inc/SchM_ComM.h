...
#define SchM_Enter_ComM_CURRENT_MODE_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_CURRENT_MODE_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_DCM_DIAGNOSTIC_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_DCM_DIAGNOSTIC_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_PNC_PREPARE_TIMER() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_PNC_PREPARE_TIMER() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_PNC_WAKEUP_IND() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_PNC_WAKEUP_IND() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_ECUM_WAKEUP_IND() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_ECUM_WAKEUP_IND() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_INHIBIT_CNT_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_INHIBIT_CNT_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_NM_INDICATION_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_NM_INDICATION_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
#define SchM_Enter_ComM_REQ_MODECNT_PROTECTION() do {\
 \
  (void) SuspendAllInterrupts(); \
} while(0);

#define SchM_Exit_ComM_REQ_MODECNT_PROTECTION() do {\
 \
  (void) ResumeAllInterrupts(); \
} while(0);
